...
public class PRIMEBigdataGraph5outkafka {
...
//						}
			...
			private double calculateSimilarity(Integer blockKey, Set<Integer> ent1, Set<Integer> ent2) {
				int maxSize = Math.min(ent1.size() - 1, ent2.size() - 1);
		...
		SingleOutputStreamOperator<String> output = graph.rebalance().map(new MapFunction<EntityNode, String>() {
			@Override
